...
Staff = \
    {
        "COMP2399": ["Amy", "Paulie", "Mark"],  # the first teacher in the list represents the lecturer
        "COMP2244": ["Owen", "John", "Tom"],
        "COMP2579": ["Natasha", "Sarah", "Max"],
    }


def create_groups(module_teachers, all_teachers, n_groups, teacher_load, max_groups=3):
    groups = {}
...
for i in range(n_events):
    for j in range(n_events):
        ...
        # RULE 1: Different modules cannot be parallel (except Personal tutorials)
        ...
        # RULE 2: Same teacher cannot teach two events at the same time
        if teacher_i == teacher_j:
            C[i][j] = HARD_PENALTY

        # # RULE 3: Labs cannot be parallel
        # if type_i == "lab" and type_j == "lab":
        #     C[i][j] = HARD_PENALTY

        # RULE 4: Same modules cannot overlap (except Tutorials)
        if module_i == module_j:
...
# Bookmark:
# analyse timetable
# listen to the latest audio too
# now assign lectures (look at all the notes in the word)
